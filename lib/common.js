...
function UpdateNavigationChildren(navigation, content) {
	$.each(navigation, function(index, object) {
		...
		if (type != undefined) {
			if(type == "search")
			{
				content.append($("<form class='form-inline' action='search.html' method='GET'><input class='form-control mr-sm-2' type='search' placeholder='Search' aria-label='Search' name='t'></form>"));
			}
			else if(type == "link")
			{
