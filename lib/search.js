	...
	$.each(filteredCards, function(index, card) {
		...
		for(var i = 0; i < reminders.length; i++)
		{
			...
			for(var j = 0; j < i; j++)
			{
				...
				if(reminders[j].text.includes(reminders[i].text))
				{
					found = true;
				}
			}
			...
			var index = rules.toUpperCase().indexOf(reminders[i].text.toUpperCase());
			...
			var string = rules.substr(index, reminders[i].text.length);
		
			var replacement = "<span class=\"rulespop\" rel=\"popover\" data-title=\"" + reminders[i].text + "\" data-content=\"" + reminders[i].description + "\"><strong>" + string + "</strong></span>";
		
			rules = rules.substr(0, index) + replacement + rules.substr(index + string.length);
		}
